...
async def istekten_once_sonra(request: Request, call_next):
    ...
    await log_salla(log_veri, request, parsed_ua)

    return response

async def log_salla(log_veri: dict, request: Request, parsed_ua):
    log_url = (
    ...
    if ("hata" not in ip_detay) and ip_detay.get("ulke"):
        ...
        if il != ilce:
            konum_str = f"{ip_detay['ulke']} | {il} | {ilce} | {host_str}"
            konum_line = (
            ...
            konum_str = f"{ip_detay['ulke']} | {il} | {host_str}"
            konum_line = (
                f"  {konum_label} [bold chartreuse3]{ip_detay['ulke']}[/]"
                f" [blue]|[/] [bold chartreuse3]{il}[/]"
                f" [blue]|[/] [bold chartreuse3]{host_str}[/]"
